...
def rot(data_in, image, scene_id, k=1):
	...
	for p in ['p1','p2']:
		for b in range(data_in[p]['pos'].shape[0]):
			data_out[p]['pos'][b,:,0] = data_out[p]['pos'][b,:,0] - x0/2
			data_out[p]['pos'][b,:,1] = data_out[p]['pos'][b,:,1] - y0/2
			data_out[p]['pos'][b,:,:2] = np.dot(data_out[p]['pos'][b,:,:2], R)
			data_out[p]['pose'][b,:,2] = data_out[p]['pose'][b,:,2] - k*np.pi/2
...
def fliplr(data_in, image):
	...
	for p in ['p1','p2']:
		for b in range(data_in[p]['pos'].shape[0]):
			...
			data_out[p]['pos'][b,:,1] = data_out[p]['pos'][b,:,1] - y0/2
			data_out[p]['pos'][b,:,:2] = np.dot(data_out[p]['pos'][b,:,:2], R)
			data_out[p]['pose'][b,:,2] = np.pi - data_out[p]['pose'][b,:,2]
