...
def rot_xy(x,y,x0,y0,x02,y02,k):
    
	x_out= x - x0 / 2
	y_out = y - y0 / 2
	c, s = np.cos(-k * np.pi / 2), np.sin(-k * np.pi / 2)
	R = np.array([[c, s], [-s, c]])
	x_out,y_out = np.dot(np.array([x_out,y_out]), R)
...
def rot(df, image, k=1):
	...
	xy.loc()[:, 'x'] = xy['x'] - x0 / 2
	xy.loc()[:, 'y'] = xy['y'] - y0 / 2
	xy.loc()[:, 'goal_x'] = xy['goal_x'] - x0 / 2
	xy.loc()[:, 'goal_y'] = xy['goal_y'] - y0 / 2
	c, s = np.cos(-k * np.pi / 2), np.sin(-k * np.pi / 2)
	R = np.array([[c, s], [-s, c]])
	xy.loc()[:, ['x', 'y']] = np.dot(xy[['x', 'y']], R)
	xy.loc()[:, ['goal_x', 'goal_y']] = np.dot(xy[['goal_x', 'goal_y']], R)
...
def rot(data_in, image, scene_id, k=1):
	...
	for p in ['p1','p2']:
		for b in list(data_in[p].keys()):
			data_out[p][b]['pos'][:,0] = data_out[p][b]['pos'][:,0] - x0/2
			data_out[p][b]['pos'][:,1] = data_out[p][b]['pos'][:,1] - y0/2
			data_out[p][b]['pos'][:,:2] = np.dot(data_out[p][b]['pos'][:,:2], R)
			data_out[p][b]['pose'][:,2] = data_out[p][b]['pose'][:,2] - k*np.pi/2

	data_out['goal'][:2] = np.dot(data_out['goal'][:2] - np.array([x0/2,y0/2]), R)
	...
	data_out['goal'][:2] = data_out['goal'][:2] + np.array([x0/2,y0/2])
    
	return data_out, image


def fliplr(data_in, image):
	...
	for p in ['p1','p2']:
		for b in list(data_in[p].keys()):
			...
			data_out[p][b]['pos'][:,1] = data_out[p][b]['pos'][:,1] - y0/2
			data_out[p][b]['pos'][:,:2] = np.dot(data_out[p][b]['pos'][:,:2], R)
			data_out[p][b]['pose'][:,2] = data_out[p][b]['pose'][:,2] + np.pi
...
def find_idx(data, scene_id):
    
    idx = []
    
    for i in range(len(data)):
        if str(data[i]['scene_id'])==str(scene_id):
            idx.append(i)
    
    return idx

def get_scene_list(data):
    
    scene_list = []
    
    for d in data:
        if not d['scene_id'] in scene_list:
...
def augment_data(data, image_path='/binary_map/', images={}, seg_mask=False, normalize_map=False):
	...
	for scene in scene_list:
		#im_path = os.path.join(image_path)
		...
		if normalize_map: 
			# images[scene] = im / 128 - 1 # normalized image from [0, 255] to [-1, 1]
			images[scene] = im / 255 # normalized image from [0, 255] to [0, 1]
		else: 
			images[scene] = im
		
	
	# data without augmentation, used so augmented data can be appended to original df
	...
	for k in ks:
		for scene in scene_list:
			...
			idx_list = find_idx(data, scene)
			for idx in idx_list:

				d_rot, im_rot = rot(data[idx], im, k)

				images[scene + rot_angle] = im_rot
                    
				# data dict
				d_rot['scene_id'] = str(scene + rot_angle)
	...
	for scene in scene_list:
		...
		idx_list = find_idx(data_, scene)
		for idx in idx_list:
			d_flip, im_flip = fliplr(data_[idx], im)
		
			# image
			images[d_flip['scene_id'] + '_fliplr'] = im_flip

			# data dict
			d_flip['scene_id'] = data_[idx]['scene_id'] + '_fliplr'
			data.append(d_flip)
...
def create_images_dict(data, image_path, seg_mask, normalize_map):
	
	#data_dir = '/media/yimengliu/f2154d17-caba-4946-930d-76959506ebc8/trajectory_prediction_dataset/vr_locomotion_dataset/processed_data'
	images = {}
	...
	for scene in scene_list:
		im_path = os.path.join(image_path)
		im_path = im_path + '{:0=3}'.format(scene) + '.png'
		...
		if normalize_map: 
			# images[scene] = im / 128 - 1 # normalized image from [0, 255] to [-1, 1]
			images['{:0=3}'.format(scene)] = im / 255 # normalized image from [0, 255] to [0, 1]
		else: 
			images['{:0=3}'.format(scene)] = im
